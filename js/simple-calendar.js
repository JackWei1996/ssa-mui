...
var LunarHelp = function () {
  function LunarHelp(year, month, day) {
    ...
    //计算年数
    for (i = 1900; i < 2099 && offset - this.lYearDays(i) > 0; i++) {
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            if (data[year + '-' + currentmonth + '-' + day]) {
            	var tt = data[year + '-' + currentmonth + '-' + day];
            	//var accept = tt.substring(tt.lenght-3,tt.lenght-1);
            	//console.log(tt);
            //设置未读显示
              if(tt.indexOf('未读')>0){
            	  v.classList.add('unread');
              }
              v.classList.add('sc-mark');
              v.classList.add('message');
    ...
    value: function updateFestival(year, month) {
      ...
      daysElement.forEach(function (v, i) {
        ...
        if (options.showFestival) {
          if (data[currentmonth + '-' + Math.floor(day)]) {
            v.querySelector('.lunar-day').innerHTML = data[currentmonth + '-' + Math.floor(day)];
            v.classList.add('sc-festival');
          }
        }
        //阴历节日
        if (lunardata && options.showLunarFestival) {
          ...
          if (lunardata[lunar.month + '-' + Math.floor(lunar.day)]) {
            v.querySelector('.lunar-day').innerHTML = lunardata[lunar.month + '-' + Math.floor(lunar.day)];
            v.classList.add('sc-festival');
          }
        }
      });
...
SimpleCalendar.prototype.languageData = {
  ...
  vocation: {
	...
	data_2017: ['1-1','1-2','1-27','1-28','1-29','1-30','1-31','2-1','2-2','4-2','4-3','4-4','4-29','4-30','5-1','5-28','5-29','5-30','10-1','10-2','10-3','10-4','10-5','10-6','10-7','10-8','12-30','12-31'],
